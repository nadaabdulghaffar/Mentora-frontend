  ...
  return (
    <aside className="rounded-2xl border border-gray-200 bg-white overflow-hidden">
      ...
      <div className="h-24 w-full bg-gray-100">
        {community.cover ? (
          <img src={community.cover} alt={community.name} className="h-full w-full object-cover" />
        ) : (
      ...
      <div className="p-4 sm:p-6">
        <div className="flex items-center gap-3">
          <img src={community.avatar} alt={community.name} className="h-12 w-12 rounded-full border-2 border-white -mt-6 bg-white object-cover" />
          <div>
            <h3 className="text-base font-semibold text-slate-900">{community.name}</h3>
            {community.domain && (
              <p className="mt-1 text-xs font-semibold text-primary uppercase tracking-wide">{community.domain}</p>
            )}
          </div>
        </div>

        <p className="mt-4 text-sm text-gray-600 leading-relaxed">{community.description}</p>
